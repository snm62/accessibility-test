class AccessibilityWidget {
    constructor() {
        this.settings = {};
        this.customization = {}; // Store customization data from KV
        ...
        this.clientId = null; // Store client ID for KV lookup
        this.siteId = null; // Store site ID for KV lookup
    ...
    async init() {
        ...
        // Fetch customization data from KV before creating widget
        ...
        setTimeout(() => {
            this.bindEvents();
            this.applySettings();
            this.applyCustomizations(); // Apply customizations after widget is created
    ...
    // KV Integration Methods
    async fetchCustomizationData() {
        try {
            // Get client ID and site ID from browser
            this.clientId = this.getClientId();
            this.siteId = this.getSiteId();
            
            if (!this.clientId || !this.siteId) {
                console.log('Accessibility Widget: No client ID or site ID found, using default settings');
                return;
            }
            
            console.log('Accessibility Widget: Fetching customization data for client:', this.clientId, 'site:', this.siteId);
            
            // Make API call to KV store
    ...
    getClientId() {
        ...
        let clientId = urlParams.get('client_id') || urlParams.get('clientId');
        
        // 2. From localStorage
        if (!clientId) {
            clientId = localStorage.getItem('accessibility_client_id');
        }
        
        // 3. From sessionStorage
        if (!clientId) {
            clientId = sessionStorage.getItem('accessibility_client_id');
        }
        
        // 4. From meta tag
        if (!clientId) {
            const metaTag = document.querySelector('meta[name="accessibility-client-id"]');
            if (metaTag) {
                clientId = metaTag.getAttribute('content');
            }
        }
        
        // 5. From data attribute on body
        if (!clientId) {
            clientId = document.body.getAttribute('data-accessibility-client-id');
        }
        
        console.log('Accessibility Widget: Client ID found:', clientId);
        return clientId;
    }
    ...
    getSiteId() {
        ...
        // 1. From URL parameter
        const urlParams = new URLSearchParams(window.location.search);
        let siteId = urlParams.get('site_id') || urlParams.get('siteId');
        ...
        // 4. From meta tag
        if (!siteId) {
            const metaTag = document.querySelector('meta[name="accessibility-site-id"]');
            if (metaTag) {
                siteId = metaTag.getAttribute('content');
            }
        }
        
        // 5. From data attribute on body
        if (!siteId) {
            siteId = document.body.getAttribute('data-accessibility-site-id');
        }
        
        // 6. From current domain as fallback
        if (!siteId) {
    ...
    applyWidgetPositioning() {
        const positioning = this.customization.positioning || {};
        
        if (this.shadowRoot && this.shadowRoot.host) {
            const widget = this.shadowRoot.host;
            
            // Horizontal position (left/right)
            if (positioning.horizontalPosition) {
                widget.style.setProperty('--widget-horizontal-position', positioning.horizontalPosition);
                if (positioning.horizontalPosition === 'left') {
                    widget.style.right = 'auto';
                    widget.style.left = '20px';
                } else if (positioning.horizontalPosition === 'right') {
                    widget.style.left = 'auto';
                    widget.style.right = '20px';
                }
            }
            ...
            // Vertical position
            if (positioning.verticalPosition) {
                widget.style.setProperty('--widget-vertical-position', positioning.verticalPosition);
                if (positioning.verticalPosition === 'top') {
                    widget.style.bottom = 'auto';
                    widget.style.top = '20px';
                } else if (positioning.verticalPosition === 'bottom') {
                    widget.style.top = 'auto';
                    widget.style.bottom = '20px';
                }
            }
            ...
            // Horizontal offset
            if (positioning.horizontalOffset !== undefined) {
                const currentPosition = positioning.horizontalPosition || 'right';
                if (currentPosition === 'left') {
                    widget.style.left = `${positioning.horizontalOffset}px`;
                } else {
                    widget.style.right = `${positioning.horizontalOffset}px`;
                }
            }
            
            // Vertical offset
            if (positioning.verticalOffset !== undefined) {
                const currentPosition = positioning.verticalPosition || 'bottom';
                if (currentPosition === 'top') {
                    widget.style.top = `${positioning.verticalOffset}px`;
                } else {
                    widget.style.bottom = `${positioning.verticalOffset}px`;
                }
            }
            
            console.log('Accessibility Widget: Widget positioning applied');
        }
    }

    applyWidgetStyling() {
        const styling = this.customization.styling || {};
        ...
        if (this.shadowRoot) {
            ...
            if (icon) {
                ...
                if (styling.backgroundColor) {
                    icon.style.backgroundColor = styling.backgroundColor;
                }
                
                // Button size
                if (styling.buttonSize) {
                    const size = styling.buttonSize;
                ...
                // Button shape
                if (styling.buttonShape) {
                    if (styling.buttonShape === 'circle') {
                        icon.style.borderRadius = '50%';
                    } else if (styling.buttonShape === 'square') {
                        icon.style.borderRadius = '0';
                    } else if (styling.buttonShape === 'rounded') {
                        icon.style.borderRadius = '8px';
                    }
                }
            }
            ...
            if (panel) {
                // Panel size
                if (styling.panelSize) {
                    const size = styling.panelSize;
                    panel.style.width = `${size}px`;
                    panel.style.maxWidth = `${size}px`;
                }
            }
            
            console.log('Accessibility Widget: Widget styling applied');
        }
    }

    applyMobileCustomizations() {
        const mobile = this.customization.mobile || {};
        ...
        if (window.innerWidth <= 768) { // Mobile breakpoint
            if (this.shadowRoot && this.shadowRoot.host) {
                ...
                // Mobile positioning
                if (mobile.horizontalPosition) {
                    if (mobile.horizontalPosition === 'left') {
                        widget.style.right = 'auto';
                        widget.style.left = '10px';
                    } else if (mobile.horizontalPosition === 'right') {
                        widget.style.left = 'auto';
                        widget.style.right = '10px';
                    }
                }
                
                if (mobile.verticalPosition) {
                    if (mobile.verticalPosition === 'top') {
                        widget.style.bottom = 'auto';
                        widget.style.top = '10px';
                    } else if (mobile.verticalPosition === 'bottom') {
                        widget.style.top = 'auto';
                ...
                // Mobile offsets
                if (mobile.horizontalOffset !== undefined) {
                    const currentPosition = mobile.horizontalPosition || 'right';
                    if (currentPosition === 'left') {
                        widget.style.left = `${mobile.horizontalOffset}px`;
                    } else {
                        widget.style.right = `${mobile.horizontalOffset}px`;
                    }
                }
                
                if (mobile.verticalOffset !== undefined) {
                    const currentPosition = mobile.verticalPosition || 'bottom';
                    if (currentPosition === 'top') {
                        widget.style.top = `${mobile.verticalOffset}px`;
                    } else {
                        widget.style.bottom = `${mobile.verticalOffset}px`;
                    }
                }
                
                // Mobile button size
                if (mobile.buttonSize) {
                    const icon = this.shadowRoot.getElementById('accessibility-icon');
                    if (icon) {
                        icon.style.width = `${mobile.buttonSize}px`;
                        icon.style.height = `${mobile.buttonSize}px`;
                        icon.style.fontSize = `${mobile.buttonSize * 0.4}px`;
                    }
                }
                
                console.log('Accessibility Widget: Mobile customizations applied');
            }
        }
    }

    applyVisibilitySettings() {
        const visibility = this.customization.visibility || {};
        
        if (visibility.hideTriggerButton === true) {
            if (this.shadowRoot) {
                const icon = this.shadowRoot.getElementById('accessibility-icon');
                if (icon) {
                    icon.style.display = 'none';
    ...
    // Method to update customizations dynamically
    async updateCustomizations() {
        await this.fetchCustomizationData();
        this.applyCustomizations();
    }

    // Method to save customizations back to KV (if needed)
    async saveCustomizations() {
        try {
            if (!this.clientId || !this.siteId) {
                console.log('Accessibility Widget: Cannot save customizations without client/site ID');
                return;
            }
            
            const response = await fetch(`${this.kvApiUrl}/customization/${this.clientId}/${this.siteId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (response.ok) {
                console.log('Accessibility Widget: Customizations saved successfully');
            } else {
                console.error('Accessibility Widget: Failed to save customizations');
            }
        } catch (error) {
            console.error('Accessibility Widget: Error saving customizations:', error);
        }
    }

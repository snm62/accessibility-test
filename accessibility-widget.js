class AccessibilityWidget {
    ...
    try {
        ...
        if (!siteId) {
            console.warn('Accessibility Widget: No siteId found, using default settings');
            return;
        }

        console.log('Accessibility Widget: Fetching customization for siteId:', siteId);
        
        const response = await fetch(`https://accessibility-widget.web-8fb.workers.dev/api/accessibility/config?siteId=${siteId}`);
        
        if (!response.ok) {
            console.warn('Accessibility Widget: Failed to fetch customization data:', response.status);
            return;
        }
        
        const config = await response.json();
        console.log('Accessibility Widget: Received customization data:', config);
        
        // Store the customization data
        this.customization = config.customization || {};
        this.accessibilityProfiles = config.accessibilityProfiles || [];
        
        // Apply the customization
        this.applyCustomization();
        
    } catch (error) {
        console.error('Accessibility Widget: Error fetching customization data:', error);
    }
}

    getSiteId() {
    // Method 1: Check for data attribute on script tag
    const scriptTag = document.querySelector('script[src*="accessibility-widget"]');
    if (scriptTag && scriptTag.dataset.siteId) {
        return scriptTag.dataset.siteId;
    }

    // Method 2: Check for data attribute on body
    if (document.body.dataset.siteId) {
        return document.body.dataset.siteId;
    }

    // Method 3: Extract from domain (if using custom domains)
    const hostname = window.location.hostname;
    if (hostname.includes('webflow.io')) {
        // Extract site name from webflow.io domain
        const match = hostname.match(/([^.]+)\.webflow\.io/);
        if (match) {
            // You'll need to map this to actual siteId
            // For now, return the site name as a fallback
            return match[1];
        }
    }
...
// ADD THIS METHOD to get siteId
    getSiteId() {
        ...
        if (hostname.includes('webflow.io')) {
            ...
            const match = hostname.match(/([^.]+)\.webflow\.io/);
            if (match) {
                return match[1]; // You might need to map this to actual siteId
            }
        }

        return null;
    }
// ADD THIS METHOD to apply customization
    applyCustomization() {
        if (!this.customization || Object.keys(this.customization).length === 0) {
            return;
        }

        console.log('Accessibility Widget: Applying customization:', this.customization);

        // Apply trigger button customization
        this.applyTriggerButtonCustomization();
        
        // Apply panel customization
        this.applyPanelCustomization();
        
        // Apply accessibility statement link
        this.applyAccessibilityStatementLink();
    }

    // ADD THIS METHOD for trigger button customization
    applyTriggerButtonCustomization() {
        const {
            triggerButtonColor,
            triggerButtonSize,
            triggerButtonShape,
        ...
        // Hide trigger button if requested
        if (hideTriggerButton === 'Yes') {
            const triggerButton = this.shadowRoot?.querySelector('#accessibility-trigger');
            if (triggerButton) {
...
// Apply button color
        if (triggerButtonColor) {
            this.updateCSSVariable('--trigger-button-color', triggerButtonColor);
        }

        // Apply button size
        if (triggerButtonSize) {
            const sizeMap = {
                'Small': '40px',
                'Medium': '50px',
                'Large': '60px'
            };
            this.updateCSSVariable('--trigger-button-size', sizeMap[triggerButtonSize] || '50px');
        }

        // Apply button shape
        if (triggerButtonShape) {
            const borderRadius = triggerButtonShape === 'Circle' ? '50%' : 
                               triggerButtonShape === 'Round' ? '8px' : '4px';
            this.updateCSSVariable('--trigger-button-border-radius', borderRadius);
        }
        // Apply positioning
        if (triggerHorizontalPosition) {
            this.updateCSSVariable('--trigger-horizontal-position', triggerHorizontalPosition.toLowerCase());
        }
        if (triggerVerticalPosition) {
            this.updateCSSVariable('--trigger-vertical-position', triggerVerticalPosition.toLowerCase());
        }

        // Apply offsets
        if (triggerHorizontalOffset) {
            this.updateCSSVariable('--trigger-horizontal-offset', triggerHorizontalOffset);
        }
        if (triggerVerticalOffset) {
            this.updateCSSVariable('--trigger-vertical-offset', triggerVerticalOffset);
        }

        // Apply mobile settings
        if (showOnMobile === 'Hide') {
            this.updateCSSVariable('--mobile-display', 'none');
        } else {
            // Apply mobile positioning and sizing
            if (mobileTriggerHorizontalPosition) {
                this.updateCSSVariable('--mobile-trigger-horizontal-position', mobileTriggerHorizontalPosition.toLowerCase());
            }
            if (mobileTriggerVerticalPosition) {
                this.updateCSSVariable('--mobile-trigger-vertical-position', mobileTriggerVerticalPosition.toLowerCase());
            }
            if (mobileTriggerSize) {
                const mobileSizeMap = {
                    'Small': '35px',
                    'Medium': '45px',
                    'Large': '55px'
                };
                this.updateCSSVariable('--mobile-trigger-size', mobileSizeMap[mobileTriggerSize] || '45px');
            }
            if (mobileTriggerShape) {
                const mobileBorderRadius = mobileTriggerShape === 'Circle' ? '50%' : 
                                         mobileTriggerShape === 'Round' ? '8px' : '4px';
                this.updateCSSVariable('--mobile-trigger-border-radius', mobileBorderRadius);
            }
            if (mobileTriggerHorizontalOffset) {
                this.updateCSSVariable('--mobile-trigger-horizontal-offset', mobileTriggerHorizontalOffset + 'px');
            }
            if (mobileTriggerVerticalOffset) {
                this.updateCSSVariable('--mobile-trigger-vertical-offset', mobileTriggerVerticalOffset + 'px');
            }
        }

        // Apply selected icon
        if (selectedIcon && selectedIcon !== 'accessibility') {
            this.updateTriggerIcon(selectedIcon);
        }
    }
// ADD THIS METHOD for panel customization
    applyPanelCustomization() {
        ...
        if (interfaceLeadColor) {
            this.updateCSSVariable('--panel-background-color', interfaceLeadColor);
        }

        // Apply panel position
        if (interfacePosition) {
            this.updateCSSVariable('--panel-position', interfacePosition.toLowerCase());
        }

        // Apply footer content
        if (interfaceFooterContent) {
            const footerElement = this.shadowRoot?.querySelector('#accessibility-footer');
            if (footerElement) {
                footerElement.innerHTML = interfaceFooterContent;
            }
            }
    }

    // ADD THIS METHOD for accessibility statement link
    applyAccessibilityStatementLink() {
        const { accessibilityStatementLink } = this.customization;
        
        if (accessibilityStatementLink) {
            const statementButton = this.shadowRoot?.querySelector('#accessibility-statement-button');
            if (statementButton) {
                statementButton.href = accessibilityStatementLink;
    ...
    updateCSSVariable(property, value) {
        if (this.shadowRoot) {
            this.shadowRoot.style.setProperty(property, value);
        }
    }
// ADD THIS METHOD to update trigger icon
    updateTriggerIcon(iconId) {
        const triggerButton = this.shadowRoot?.querySelector('#accessibility-trigger');
        if (triggerButton) {
            // You'll need to implement icon mapping based on your icon options
            const iconMap = {
                ...
                'brain': 'fas fa-brain',
                'ad-triple': 'fas fa-ad' // Add your custom icons here
            };
            
            const iconClass = iconMap[iconId] || 'fas fa-universal-access';
            const iconElement = triggerButton.querySelector('i');
            if (iconElement) {
